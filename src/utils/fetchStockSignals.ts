// 종목별 기술적 신호 (MA20/MA60/60일 고저점 기반)
...
export interface StockSignal {
  ...
  high: number;   // 60일 고점
  low: number;    // 60일 저점
  position: number; // 0=저점 1=고점 (60일 range 내 위치)
}
...
export function getSellSignal(s: StockSignal): SignalResult {
  ...
  if (trend === 'up' && pos >= 0.7)
    return { label: '매도 적합', color: 'var(--color-profit)', desc: `상승 추세 고점권 (60일 중${(pos*100).toFixed(0)}% 위치). 지금이 최적 매도 타이밍. 더 기다리면 고점을 지나 하락 전환 위험이 있으니 분할 매도 고려`, range: [s.currentPrice, s.high] };
  if (trend === 'up')
    return { label: '매도 가능', color: 'var(--color-profit)', desc: `상승 추세이나 고점까지 여지 있음 (현재 ${(pos*100).toFixed(0)}% / 70% 기준). 60일 고점 근처까지 기다리면 더 비싸게 매도 가능. 급하게 팔 필요 없다면 조금 더 보유 권장`, range: [s.currentPrice, p70] };
...
export function getBuySignal(s: StockSignal): SignalResult {
  ...
  if (trend === 'down' && pos <= 0.2)
    return { label: '반등 대기', color: 'var(--color-warning)', desc: `하락 추세 + 60일 저점 근처 (${(pos*100).toFixed(0)}%). 지금 사면 더 내려갈 수 있음. MA20 회복 신호가 나올 때까지 기다렸다가 진입이 손실 최소화에 유리`, range: [s.low, s.ma20] };
  if (trend === 'down')
  ...
  if (trend === 'up' && pos >= 0.8)
    return { label: '조정 대기', color: 'var(--color-loss)', desc: `60일 고점 근처 (${(pos*100).toFixed(0)}%). 지금 사면 고점 매수 위험. 10~15% 조정 후 MA20~MA60 구간까지 내려올 때 진입하면 훨씬 유리한 가격`, range: [s.ma60, s.ma20] };
  if (trend === 'up' && pos >= 0.6)
